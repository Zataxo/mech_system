...
const { isEmpty,isListContainEmptyVal } = require("../../../utils/utilities");
...
class LoadService {
    ...
    createProjectConfiguration(req,res){
        ...
        if(isListContainEmptyVal([country,city])){
        ...
        let projConfig = {
            ...
            upsEffiency: req.body.upsEffiency??0.0,
    ...
    getAllProjectConfigurations(req,res){
        models.ProjectConfiguration.findAll({}).then(projectConfigs =>{
    ...
    createProject(req,res){
        ...
        const {lProjName} = req.body;
        if(isEmpty(lProjName)){
            return this.selfFinalizer.failureReq(res, "Bad request missing arguments", 400);
        }
        let lProj =  {
            lProjName:lProjName,
    ...
    getAllProjects(req,res){
        models.LoadProjects.findAll({
            ...
            this.selfFinalizer.successReq(res, projects,"Projects Fetched",200);        }).catch(err=>{
            this.selfFinalizer.failureReq(res, err, 500);
        })
    }
